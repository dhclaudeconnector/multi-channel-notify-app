# Claude Code Project Rules (dockerstackds2api)

These rules are mandatory for Claude Code in this project.

## 1) Always update `.opushforce.message`

- For every completed user task, Claude Code must update `.opushforce.message` before sending the final response.
- Do not skip this step, even for small edits.
- If no files were changed, still update `.opushforce.message` with a short note.

## 2) Message format

Use conversational summary content, aligned with what Claude Code says to the user.
...
Claude Code should treat task as incomplete until `.opushforce.message` is updated to reflect the latest work. When updating `.opushforce.message`:

- If the file already exists and contains content, append the new message to the end (separated by a newline)
- If the file is empty or doesn't exist, create it with the new content
